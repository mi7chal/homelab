...
| **VIP 10.0.0.3 NIE odpowiada** — ani ping, ani porty | `ping` timeout, `nc` timeout |
| **Brak ARP entry** dla 10.0.0.3 | `arp -a` nie zwraca wyniku dla tego IP |
...
| MetalLB serwis poprawnie skonfigurowany | `home-pool: 10.0.0.3-10.0.0.3`, L2 advertisement OK |

**Wniosek**: Traefik działa poprawnie. Problem jest w **MetalLB** — speaker na control-pane (który jest odpowiedzialny za ogłaszanie VIP 10.0.0.3 przez gratuitous ARP) przestał wysyłać ARP announcements. Bez ARP, sieć nie wie, że 10.0.0.3 jest osiągalny na MAC adresie control-pane.
...
2. Sprawdzić, czy po restarcie `ping 10.0.0.3` zaczyna działać
3. Sprawdzić, czy `curl -sk https://10.0.0.3` zwraca odpowiedź Traefika
